  ...
  <section class="content">
    <div class="box box-default">
        ...
        <div class="box-body">
          <div class="row">
            <div class="col-md-4">
              <form class="" action="{{url($prefix.'/assessment/sectionone')}}" method="post" id="table">
                {{csrf_field()}}
                <div class="form-group">
                  <label>Kandidat</label>
                  <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" required="" name="interview_id">
                    <option value="">Pilih</option>
                    @foreach ($inter as $row)
                      <option value="{{$row->id}}">{{$row->Candidate->number}} - {{$row->Candidate->name}}</option>
                    @endforeach
                  </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <label>Fashion Show</label>
                  <input type="number" name="fashion_show" value="" class="form-control" min="1" max="100" required="">
                </div>
                <div class="form-group">
                  <label>Catwalk</label>
